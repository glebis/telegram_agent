            ...
            if connections:
                for conn in connections:
                    if (
                        ...
                        typer.echo(
                            f"🔥 Killing process {proc.info['name']} (PID: {proc.info['pid']}) using port {port}"
                        )
                        proc.kill()
                        killed_count += 1
                        break
...
def start(
    ...
    try:
        ...
        try:
            ...
            typer.echo(
                f"⚠️  Could not inspect port {port} ({e}); continuing with basic check"
            )
                ...
                if killed > 0:
                    ...
                    if is_port_in_use(port):
                        ...
                        if auto_port:
                            ...
                            typer.echo(
                                "💡 Try running with --kill-existing or --auto-port flags"
                            )
                            raise typer.Exit(1)
                else:
                    ...
                    if auto_port:
                        ...
                        typer.echo(
                            "💡 Try running with --auto-port flag to use a different port"
                        )
                        raise typer.Exit(1)
            elif auto_port:
                port = find_free_port(original_port + 1)
                            ...
                            try:
                                ...
                                if health_response.status_code == 200:
                                    ...
                                    if health_data.get("status") in [
                                        ...
                                        typer.echo(
                                            f"🌡️  Health check: {health_data.get('status', 'unknown')}"
                                        )
                                    else:
                                        typer.echo(
                                            "⚠️  Health check returned unexpected status"
                                        )
                                else:
                                    typer.echo(
...
async def setup_development_webhook(
    ...
    try:
        ...
        if provider is None:
            typer.echo("⏭️  No tunnel provider configured, skipping")
            return

        typer.echo(f"🔗 Setting up {provider.name} tunnel and webhook...")
        ...
        if success:
            typer.echo("✅ Webhook setup successful!")

            # Prominent webhook URL display
            typer.echo("\n" + "=" * 60)
            typer.echo(f"🤖 TELEGRAM WEBHOOK CONFIGURED ({provider.name})")
            typer.echo(f"🔗 Webhook URL: {webhook_url}")
            typer.echo("🎯 Bot ready to receive messages!")
            typer.echo("=" * 60 + "\n")
...
def stop(
    ...
    for proc in psutil.process_iter(["pid", "name"]):
        try:
            if proc.info["name"] and "cloudflared" in proc.info["name"].lower():
                proc.kill()
                total_killed += 1
                typer.echo(f"🔥 Killed cloudflared process (PID: {proc.info['pid']})")
            ...
            if proc.info["cmdline"] and any(
                ...
                if port is not None:
                    if f"--port {port}" in " ".join(
                        ...
                        typer.echo(
                            f"🔥 Killing uvicorn process on port {port} (PID: {proc.info['pid']})"
                        )
                        proc.kill()
                        killed_uvicorn += 1
                else:
    ...
    if total_killed > 0:
        typer.echo(
            f"✅ Development environment stopped ({total_killed} processes killed)"
        )
    else:
...
def webhook():
    ...
    typer.echo(
        "🔗 Webhook management - use the FastAPI admin interface at /admin/webhook"
    )
...
def status():
    ...
    try:
        ...
        if provider:
            ...
            if url:
                ngrok_url = url
                typer.echo(f"✅ {provider.name}: tunnel active")
                typer.echo(
                    f"   🔗 {url} -> http://localhost:{os.getenv('TUNNEL_PORT', '8000')}"
                )
            else:
                ...
                if response.status_code == 200:
                    ...
                    if tunnels:
                        ...
                        for tunnel in tunnels:
                            ...
                            typer.echo(
                                f"   🔗 {tunnel['public_url']} -> {tunnel['config']['addr']}"
                            )
                    else:
                        typer.echo(
                            f"⚠️  {tunnel_provider_name}: Running but no tunnels"
                        )
                else:
                    typer.echo(f"⚠️  {tunnel_provider_name}: Not active")
    except Exception:
        typer.echo(f"❌ {tunnel_provider_name}: Not running")
    ...
    if telegram_status:
        ...
        if webhook_url:
            ...
            if ngrok_url and ngrok_url not in webhook_url:
                ...
                typer.echo(
                    f"   🔧 Run: python scripts/setup_webhook.py auto-update --port 8000"
                )
        else:
            typer.echo("❌ Telegram webhook: NOT CONFIGURED")
            typer.echo(
                "   🔧 Run: python scripts/setup_webhook.py auto-update --port 8000"
            )
