...
class Message(Base, TimestampMixin):
    ...
    # Message content
    message_type: Mapped[str] = mapped_column(String(20), nullable=False)  # text, photo, document, etc.
    ...
    image_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("images.id"), nullable=True)
    
    # Admin tracking
    admin_sent: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)  # Sent manually by admin
    admin_user: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)  # Admin who sent the message
