...
def setup_trail_scheduler(application: Application) -> None:
    ...
    if not config.is_enabled():
        logger.info("Trail review scheduler disabled (TRAIL_REVIEW_ENABLED=false or no chat ID)")
        return
    ...
    for poll_time in config.get_poll_times():
        ...
        logger.info(f"📅 Scheduled trail review at {poll_time.hour}:{poll_time.minute:02d}")

    logger.info(f"✅ Trail review scheduler configured with {len(config.get_poll_times())} daily polls")
