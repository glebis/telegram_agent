        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                try:
                    ...
                    await db.commit()
                    logger.info(f"Stored embedding for image {image_id} (fallback mode)")
                    return True
                ...
                await db.commit()
                logger.info(f"Stored embedding for image {image_id} with embedding_id {embedding_id}")
                return True
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                try:
                    ...
                    params = [embedding_array.tobytes(), user_id, limit * 2]  # Get more results to filter
                    ...
                    logger.info(f"Found {len(filtered_results)} similar images using vector search")
                    ...
                    return await self._fallback_similarity_search(
                        db, embedding_bytes, user_id, chat_id, limit, similarity_threshold
                    )
