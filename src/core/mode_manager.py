...
class ModeManager:
    ...
    def get_mode_prompt(self, mode_name: str, preset_name: Optional[str] = None) -> str:
        ...
        if preset_info:
            return preset_info.get("prompt", mode_info.get("prompt", "Describe this image."))
            ...
            if presets and not isinstance(presets, list):
                ...
                for i, preset in enumerate(presets):
                    if not isinstance(preset, dict):
                        errors.append(f"Mode '{mode_name}' preset {i} is not a dictionary")
                        continue
                    
                    if "name" not in preset:
                        errors.append(f"Mode '{mode_name}' preset {i} missing 'name' field")
                    
                    if "prompt" not in preset:
                        errors.append(f"Mode '{mode_name}' preset {i} missing 'prompt' field")
        
        # Validate aliases
        aliases = self._config.get("aliases", {})
        for alias, target in aliases.items():
            if "." not in target:
                errors.append(f"Alias '{alias}' target '{target}' should be in 'mode.preset' format")
            else:
                mode_name, preset_name = target.split(".", 1)
                if not self.is_valid_mode(mode_name):
                    errors.append(f"Alias '{alias}' references invalid mode '{mode_name}'")
                elif preset_name and not self.is_valid_preset(mode_name, preset_name):
                    errors.append(f"Alias '{alias}' references invalid preset '{preset_name}' for mode '{mode_name}'")
