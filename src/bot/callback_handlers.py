...
async def handle_reanalyze_callback(query, file_id, params) -> None:
    ...
    logger.info(f"Reanalyzing image {file_id} with mode: {new_mode}, preset: {new_preset}")
    ...
    try:
        ...
        if cached_analysis:
            logger.info(f"Using cached reanalysis for file_id={file_id}, mode={new_mode}, preset={new_preset}")
            
            # Format cached response
            response_text, reply_markup = llm_service.format_telegram_response(cached_analysis, include_keyboard=True)
            ...
            await query.edit_message_text(
                "✅ Analysis completed! Check the new message above.",
                parse_mode="HTML"
            )
            return
        ...
        response_text, reply_markup = llm_service.format_telegram_response(analysis, include_keyboard=True)
        ...
        # Update the original message to show it was processed
        await query.edit_message_text(
            "✅ Re-analysis completed! Check the new message above.",
            parse_mode="HTML"
        )
        ...
        try:
            async with get_db_session() as session:
                ...
                logger.info(f"Saved reanalysis for file_id: {file_id}, image_id: {saved_image_id}")
                
                # Store embedding in vector database for efficient similarity search
                if embedding_bytes:
                    try:
                        success = await vector_db.store_embedding(saved_image_id, embedding_bytes)
                        if success:
                            logger.info(f"Stored embedding in vector database for image {saved_image_id}")
                        else:
                            logger.warning(f"Failed to store embedding in vector database for image {saved_image_id}")
                    except Exception as e:
    ...
    try:
        ...
        async with get_db_session() as session:
            ...
            if chat_record:
                ...
                keyboard_utils = get_keyboard_utils()
                reply_markup = keyboard_utils.create_mode_selection_keyboard(new_mode, new_preset)
